...
namespace IndoorMapTools.Core
{
    ...
    {
        private class MSTerminator
        {
            private readonly MemoryStream stream;
            public MSTerminator(MemoryStream stream) => this.stream = stream;
            ~MSTerminator() => stream.Dispose();
        }

        private static readonly ConditionalWeakTable<BitmapImage, MSTerminator> msTable = new ConditionalWeakTable<BitmapImage, MSTerminator>();
        ...
        public static BitmapImage BitmapImageFromMemoryStream(MemoryStream stream)
        {
            ...
            // 메모리 스트림 약한참조 등록
            msTable.Add(image, new MSTerminator(stream));
